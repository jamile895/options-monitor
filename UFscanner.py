...
def parse_and_filter(raw: list[dict], underlying: float, ticker: str) -> pd.DataFrame:
    ...
    for item in raw:
        ...
        # Polygon snapshot: last_quote usa bid_price/ask_price (non bid/ask)
        bid = (quotes.get("bid_price") or quotes.get("bid") or 0)
        ask = (quotes.get("ask_price") or quotes.get("ask") or 0)

        # Fallback chain: day close -> vwap -> last trade price
        day_close  = day.get("close")  or 0
        day_vwap   = day.get("vwap")   or 0
        last_price = last_trade.get("price") or 0
        ...
        elif last_price > 0:       mid = last_price
        else:                      mid = 0
