...
# =========================
with st.expander("📖 Manuale — Options Flow Scanner PRO v4.4.6"):
    st.markdown("""
## 🎯 Obiettivo del Tool
...
if st.button("🚀 Scansiona mercato", type="primary", use_container_width=True):
    ...
    if not final_df.empty:
        # Salva in session_state per usarlo fuori dal blocco scan
        st.session_state["last_scan_df"]   = final_df.copy()
        st.session_state["last_scan_mode"] = mode
...
# =========================
# AGGIUNGI A WATCHLIST — fuori dal blocco scan per evitare rerun
# =========================
if "last_scan_df" in st.session_state and not st.session_state["last_scan_df"].empty:
    last_df = st.session_state["last_scan_df"]
    if "OPZIONE" in last_df.columns:
        with st.expander("⭐ Aggiungi risultati alla Watchlist", expanded=False):
            opzioni_disponibili = last_df["OPZIONE"].tolist()
            sel_opzioni = st.multiselect(
                "Seleziona una o più opzioni da aggiungere:",
                options=opzioni_disponibili,
                placeholder="Scegli le opzioni...",
                key="multisel_wl_stable"
            )
            if st.button("➕ Aggiungi alla Watchlist", key="add_wl_stable", type="secondary"):
                added_count = 0
                for opzione in sel_opzioni:
                    matches = last_df[last_df["OPZIONE"] == opzione]
                    if matches.empty:
                        continue
