...
def parse_and_filter(raw: list[dict], underlying: float, ticker: str) -> pd.DataFrame:
    ...
    # SIGNAL
    def signal(x):
        if x >= 1_000_000: return "🟢 GO"
        if x >= 300_000:   return "🟡 HOLD"
        return "🔴 STOP"
    ...
    df.attrs["pc_ratio"] = round(puts / calls, 2) if calls > 0 else 0
    df.attrs["calls_vol"] = int(calls)
    df.attrs["puts_vol"]  = int(puts)
