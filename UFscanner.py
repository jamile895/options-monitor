...
def get_underlying_price_polygon(ticker: str) -> float | None:
    # Metodo 1: snapshot opzioni — prezzo da day.close del primo contratto
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if results:
                # prova prima underlying_asset.price, poi day fields del sottostante
                ua = results[0].get("underlying_asset", {})
                price = (
                    ua.get("price") or
                    ua.get("close") or
                    ua.get("value")
        ...
        pass
    # Metodo 2: aggregati giornalieri (prev close)
    try:
        ...
        if r2.status_code == 200:
            ...
            if results2:
                return round(float(results2[0]["c"]), 2)
    except Exception:
        pass
    # Metodo 3: last trade
    try:
        url3 = f"https://api.polygon.io/v2/last/trade/{ticker}"
        params3 = {"apiKey": POLYGON_API_KEY}
        r3 = requests.get(url3, params=params3, timeout=8)
        if r3.status_code == 200:
            res3 = r3.json().get("results", {})
            price3 = res3.get("p") or res3.get("c")
            if price3:
