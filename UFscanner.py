    ...
    if wl:
        ...
        if st.button("🔄 Aggiorna tutti i contratti in watchlist", key="wl_refresh"):
            ...
            for i, entry in enumerate(wl):
                ...
                try:
                    ...
                    if match:
                        ...
                        # v4.6 — salva snapshot giornaliero su wl_history
            ...
            if wl_results:
                ...
                # v4.6 — Storico andamento per contratto
                st.markdown("---")
                st.markdown("**📈 Storico andamento contratto:**")
                wl_labels_hist = [r["Contratto"] for r in wl_results]
                sel_hist = st.selectbox("Seleziona contratto:", wl_labels_hist, key="wl_hist_sel")
                if sel_hist:
                    sel_e = next((e for e in wl if
                        f"{e.get('ticker','')} {e.get('expiration','')} {e.get('strike','')}{e.get('type','')}"==sel_hist),None)
                    if sel_e:
                        hist_data = load_watchlist_history(sel_e.get("ticker",""),sel_e.get("strike",""),
                                                           sel_e.get("expiration",""),sel_e.get("type",""))
                        if hist_data:
                            df_hw = pd.DataFrame(hist_data)
                            df_hw["date"] = pd.to_datetime(df_hw["date"])
                            df_hw = df_hw.sort_values("date")
                            for col in ["mid","voi","iv","volume","underlying"]:
                                try: df_hw[col] = pd.to_numeric(df_hw[col], errors="coerce")
                                except: pass
                            c1,c2,c3,c4 = st.columns(4)
                            def delta_str(col, fmt):
                                try:
                                    v = df_hw[col].dropna()
                                    if len(v)>=2: return fmt.format(v.iloc[-1]-v.iloc[-2])
                                except: pass
                                return None
                            c1.metric("MID", f"${df_hw['mid'].iloc[-1]:.2f}" if pd.notna(df_hw['mid'].iloc[-1]) else "—", delta=delta_str("mid","${:.2f}"))
                            c2.metric("VOI", f"{df_hw['voi'].iloc[-1]:.2f}" if pd.notna(df_hw['voi'].iloc[-1]) else "—", delta=delta_str("voi","{:.2f}"))
                            c3.metric("IV%", f"{df_hw['iv'].iloc[-1]:.1f}%" if pd.notna(df_hw['iv'].iloc[-1]) else "—", delta=delta_str("iv","{:.1f}%"))
                            c4.metric("Underlying", f"${df_hw['underlying'].iloc[-1]:.2f}" if pd.notna(df_hw['underlying'].iloc[-1]) else "—", delta=delta_str("underlying","${:.2f}"))
                            df_show = df_hw[["date","underlying","mid","voi","iv","volume"]].copy()
                            ...
                            st.caption(f"📊 {len(df_hw)} giorni di storico per {sel_hist}")
                        else:
...
if st.button("🚀 Scansiona mercato", type="primary", use_container_width=True):
    ...
    if not final_df.empty:
        ...
        st.markdown("**⭐ Seleziona opzioni da aggiungere alla Watchlist:**")
        # Prepara tabella con checkbox
        grid_for_wl = final_df[["OPZIONE"]].reset_index(drop=True).copy()
        grid_for_wl.insert(0, "⭐", False)
        edited = st.data_editor(
            grid_for_wl,
            column_config={"⭐": st.column_config.CheckboxColumn("⭐", default=False, width="small"),
                           "OPZIONE": st.column_config.TextColumn("Opzione", disabled=True)},
            hide_index=True,
            use_container_width=True,
            key="wl_checkbox_editor"
        )
        if st.button("➕ Aggiungi selezionate alla Watchlist", key="add_to_wl_btn", type="secondary"):
            selected_rows = edited[edited["⭐"] == True]
            added_count = 0
            for idx in selected_rows.index:
                row_data = final_df.iloc[idx]
                # Estrai dati dal contratto
                opzione   = str(row_data.get("OPZIONE",""))
                ticker_wl = opzione.split()[0] if opzione else ""
                strike_wl = row_data.get("strike")
                exp_wl    = row_data.get("exp_str","")
                ctype     = row_data.get("type","")
                type_wl   = "C" if ctype == "CALL" else "P"
                note_wl   = f"Flow {row_data.get('FLOW $','')} | VOI {row_data.get('VOI','')}"
                if ticker_wl and strike_wl and exp_wl:
            ...
            if added_count > 0:
                ...
                st.warning("⚠️ Seleziona almeno un'opzione con la checkbox ⭐")
            else:
                st.info("ℹ️ Contratti già presenti in watchlist.")
