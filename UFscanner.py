...
def get_underlying_price_polygon(ticker: str) -> float | None:
    # Usa snapshot opzioni: previous_close e close dal campo day
    url = f"https://api.polygon.io/v3/snapshot/options/{ticker}"
    params = {"apiKey": POLYGON_API_KEY, "limit": 10}
    try:
        ...
        if r.status_code == 200:
            ...
            for item in results:
                price = item.get("day", {}).get("previous_close")
                if price and float(price) > 1:
                    return round(float(price), 2)
                price2 = item.get("day", {}).get("close")
                if price2 and float(price2) > 1:
