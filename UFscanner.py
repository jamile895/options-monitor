...
def parse_and_filter(raw: list[dict], underlying: float, ticker: str) -> pd.DataFrame:
    ...
    for item in raw:
        ...
        # bid/ask da last_quote (funziona con Options Starter per contratti liquidi)
        bid = quotes.get("bid") or quotes.get("bid_price") or 0
        ask = quotes.get("ask") or quotes.get("ask_price") or 0

        # MID: bid/ask se disponibili, altrimenti day.close poi vwap
        day_close = day.get("close") or 0
        day_vwap  = day.get("vwap")  or 0
        if   bid > 0 and ask > 0: mid = round((bid + ask) / 2, 2)
        elif ask > 0:              mid = ask
        elif bid > 0:              mid = bid
        elif day_close > 0:        mid = day_close
        elif day_vwap  > 0:        mid = day_vwap
        else:                      mid = 0
...
# =========================
with st.expander("📖 Manuale — Options Flow Scanner PRO v4.1.5"):
    st.markdown("""
## 🎯 Obiettivo del Tool
