                ...
                if sel_hist:
                    ...
                    if sel_e:
                        ...
                        if hist_data:
                            ...
                            def safe_delta(col, fmt):
                                try:
                                    v = df_hw[col].dropna()
                                    if len(v)>=2: return fmt.format(v.iloc[-1]-v.iloc[-2])
                                except: pass
                                return None
                            c1.metric("MID", f"${df_hw['mid'].iloc[-1]:.2f}" if pd.notna(df_hw['mid'].iloc[-1]) else "—", delta=safe_delta("mid","${:.2f}"))
                            c2.metric("VOI", f"{df_hw['voi'].iloc[-1]:.2f}" if pd.notna(df_hw['voi'].iloc[-1]) else "—", delta=safe_delta("voi","{:.2f}"))
                            c3.metric("IV%", f"{df_hw['iv'].iloc[-1]:.1f}%" if pd.notna(df_hw['iv'].iloc[-1]) else "—", delta=safe_delta("iv","{:.1f}%"))
                            c4.metric("Underlying", f"${df_hw['underlying'].iloc[-1]:.2f}" if pd.notna(df_hw['underlying'].iloc[-1]) else "—", delta=safe_delta("underlying","${:.2f}"))
...
if st.button("🚀 Scansiona mercato", type="primary", use_container_width=True):
    ...
    if not final_df.empty:
        ...
        # v4.5 — Aggiungi a Watchlist con multiselect (stabile, no rerun)
        st.markdown("**⭐ Aggiungi alla Watchlist:**")
        opzioni_disponibili = final_df["OPZIONE"].tolist() if "OPZIONE" in final_df.columns else []
        sel_opzioni = st.multiselect(
            "Seleziona una o più opzioni:",
            options=opzioni_disponibili,
            placeholder="Scegli le opzioni da aggiungere...",
            key="multisel_wl"
        )
        if st.button("➕ Aggiungi selezionate alla Watchlist", key="add_to_wl_btn", type="secondary"):
            added_count = 0
            for opzione in sel_opzioni:
                row_data = final_df[final_df["OPZIONE"] == opzione].iloc[0]
                ticker_wl = str(opzione).split()[0]
            ...
            elif len(sel_opzioni) == 0:
                st.warning("⚠️ Seleziona almeno un'opzione dal menu.")
            else:
