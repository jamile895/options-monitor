...
def scan_ticker(ticker: str) -> pd.DataFrame:
    ...
    # DEBUG — mostra struttura raw primo contratto
    with st.expander("🔧 DEBUG — struttura raw primo contratto Polygon"):
        if raw:
            import json
            first = raw[0]
            st.json(first)
            # Mostra anche le chiavi di secondo livello
            st.write("**Chiavi top level:**", list(first.keys()))
            st.write("**last_quote keys:**", list(first.get("last_quote", {}).keys()))
            st.write("**last_trade keys:**", list(first.get("last_trade", {}).keys()))
            st.write("**day keys:**", list(first.get("day", {}).keys()))
...
# =========================
with st.expander("📖 Manuale — Options Flow Scanner PRO v4.1.3"):
    st.markdown("""
## 🎯 Obiettivo del Tool
...
st.caption(
    ...
    "Nessun ordine viene eseguito automaticamente. — v4.1.1"
)
