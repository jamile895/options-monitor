...
# Storico giornaliero watchlist — un record per ogni aggiornamento
...
def enrich_with_flow_data(df: pd.DataFrame, ticker: str, top_n: int = 15) -> pd.DataFrame:
    ...
    # Scrivi tutto lo storico accumulato su GSheets in una sola chiamata
...
def save_watchlist_snapshot(ticker, strike, expiration, contract_type, mid, voi, iv, volume, underlying):
    """Salva uno snapshot giornaliero del contratto in watchlist su GSheet 'wl_history'."""
    today_str = datetime.today().strftime("%Y-%m-%d")
    ...
    if sheet:
        ...
        for attempt in range(3):
            try:
                ...
                # Controlla se esiste già un record oggi per questo contratto
                ...
                row = [today_str, str(ticker), str(strike), str(expiration),
                       str(contract_type),
...
def load_watchlist_history(ticker=None, strike=None, expiration=None, contract_type=None) -> list:
    """Carica storico snapshot watchlist, opzionalmente filtrato per contratto."""
    sheet = get_sheet("wl_history")
    rows = []
    if sheet:
        try:
    ...
    if ticker:
        rows = [r for r in rows if
                str(r.get("ticker",""))==str(ticker) and
                str(r.get("strike",""))==str(strike) and
                str(r.get("expiration",""))==str(expiration) and
    ...
    if wl:
        ...
        if st.button("🔄 Aggiorna tutti i contratti in watchlist", key="wl_refresh"):
            ...
            for i, entry in enumerate(wl):
                ...
                try:
                    ...
                    if match:
                        ...
                        # v4.6 — Salva snapshot giornaliero
            ...
            if wl_results:
                ...
                # v4.6 — Storico andamento per contratto selezionato
                st.markdown("---")
                st.markdown("**📈 Storico andamento contratto:**")
                wl_labels_hist = [r["Contratto"] for r in wl_results]
                sel_hist = st.selectbox("Seleziona contratto da analizzare:", wl_labels_hist, key="wl_hist_sel")
                if sel_hist:
                    sel_entry = next((e for e in wl if
                        f"{e.get('ticker','')} {e.get('expiration','')} {e.get('strike','')}{e.get('type','')}" == sel_hist), None)
                    if sel_entry:
                        hist_data = load_watchlist_history(
                            sel_entry.get("ticker",""), sel_entry.get("strike",""),
                            sel_entry.get("expiration",""), sel_entry.get("type","")
                        )
                        if hist_data:
                            df_hist_wl = pd.DataFrame(hist_data)
                            df_hist_wl["date"] = pd.to_datetime(df_hist_wl["date"])
                            df_hist_wl = df_hist_wl.sort_values("date")
                            for col in ["mid","voi","iv","volume"]:
                                try: df_hist_wl[col] = pd.to_numeric(df_hist_wl[col], errors="coerce")
                                except: pass

                            col_m1, col_m2, col_m3, col_m4 = st.columns(4)
                            if "mid" in df_hist_wl.columns:
                                col_m1.metric("MID oggi", f"${df_hist_wl['mid'].iloc[-1]:.2f}" if pd.notna(df_hist_wl['mid'].iloc[-1]) else "—",
                                    delta=f"${df_hist_wl['mid'].iloc[-1]-df_hist_wl['mid'].iloc[-2]:.2f}" if len(df_hist_wl)>1 and pd.notna(df_hist_wl['mid'].iloc[-2]) else None)
                            if "voi" in df_hist_wl.columns:
                                col_m2.metric("VOI oggi", f"{df_hist_wl['voi'].iloc[-1]:.2f}" if pd.notna(df_hist_wl['voi'].iloc[-1]) else "—",
                                    delta=f"{df_hist_wl['voi'].iloc[-1]-df_hist_wl['voi'].iloc[-2]:.2f}" if len(df_hist_wl)>1 and pd.notna(df_hist_wl['voi'].iloc[-2]) else None)
                            if "iv" in df_hist_wl.columns:
                                col_m3.metric("IV oggi", f"{df_hist_wl['iv'].iloc[-1]:.1f}%" if pd.notna(df_hist_wl['iv'].iloc[-1]) else "—",
                                    delta=f"{df_hist_wl['iv'].iloc[-1]-df_hist_wl['iv'].iloc[-2]:.1f}%" if len(df_hist_wl)>1 and pd.notna(df_hist_wl['iv'].iloc[-2]) else None)
                            if "volume" in df_hist_wl.columns:
                                col_m4.metric("Vol oggi", format_k(df_hist_wl['volume'].iloc[-1]) if pd.notna(df_hist_wl['volume'].iloc[-1]) else "—")

                            # Tabella storico completo
                            df_show = df_hist_wl[["date","underlying","mid","voi","iv","volume"]].copy()
                            df_show["date"] = df_show["date"].dt.strftime("%Y-%m-%d")
                            df_show.columns = ["Data","Underlying","MID","VOI","IV%","Volume"]
                            st.dataframe(df_show.sort_values("Data",ascending=False), use_container_width=True, hide_index=True)
                            st.caption(f"📊 {len(df_hist_wl)} giorni di storico per {sel_hist}")
                        else:
                            st.info("Nessuno storico ancora — clicca 'Aggiorna' ogni giorno per costruirlo.")
        else:
...
if st.button("🚀 Scansiona mercato", type="primary", use_container_width=True):
    ...
    if not final_df.empty:
        ...
        # v4.5 — Aggiungi a Watchlist dalla griglia
        st.markdown("**⭐ Aggiungi alla Watchlist:**")
        grid_for_wl = final_df[["OPZIONE","type","strike","exp_str"]].reset_index(drop=True).copy()
        grid_for_wl.insert(0, "⭐", False)
        ...
        selected_idx = edited[edited["⭐"]].index.tolist()
        if st.button("➕ Aggiungi selezionate alla Watchlist", key="add_selected_wl"):
            added_count = 0
            for idx in selected_idx:
                row_data = final_df.iloc[idx]
                ticker_wl = row_data.get("OPZIONE","").split()[0]
                strike_wl = row_data.get("strike")
                exp_wl    = row_data.get("exp_str","")
                type_wl   = "C" if row_data.get("type","") == "CALL" else "P"
                note_wl   = f"Flow {row_data.get('FLOW $','')} | VOI {row_data.get('VOI','')}"
                ok = add_to_watchlist(ticker_wl, strike_wl, exp_wl, type_wl, note_wl)
                if ok: added_count += 1
            if added_count > 0:
                st.success(f"✅ {added_count} contratt{'o' if added_count==1 else 'i'} aggiunt{'o' if added_count==1 else 'i'} alla Watchlist!")
            else:
                st.info("Seleziona almeno un contratto con la checkbox ⭐")
