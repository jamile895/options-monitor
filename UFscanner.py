...
def get_iv_rank(ticker: str, strike: float, expiration: str, contract_type: str, iv_current: float) -> str:
    """
    Calcola IV Rank da storico JSON.
    Formula: (IV oggi - IV min storico) / (IV max storico - IV min storico) * 100
    Richiede almeno 5 osservazioni per essere affidabile.
    """
    history = load_history()
    iv_values = [
        e["iv"] for e in history
        if (e["ticker"] == ticker and
            str(e["strike"]) == str(strike) and
            e["expiration"] == expiration and
            e["type"] == contract_type and
            e.get("iv") is not None)
    ]

    # Aggiungi IV corrente
    if iv_current is not None:
        iv_values.append(float(iv_current))

    if len(iv_values) < 3:
        return f"~{int(iv_current)}% (dati insuff.)" if iv_current else "—"

    iv_min = min(iv_values)
    iv_max = max(iv_values)

    if iv_max == iv_min:
        return f"50% ({len(iv_values)}d)"

    rank = round((iv_current - iv_min) / (iv_max - iv_min) * 100, 0)
    rank = max(0, min(100, rank))
    emoji = "🔴" if rank >= 70 else ("🟢" if rank <= 30 else "🟡")
    return f"{emoji} {int(rank)}% ({len(iv_values)}d)"
...
def add_to_watchlist(ticker: str, strike: float, expiration: str, contract_type: str, note: str = ""):
    ...
    key = (ticker, str(strike), expiration, contract_type)
    existing = {(e["ticker"], str(e["strike"]), e["expiration"], e["type"]) for e in wl}
...
def enrich_with_flow_data(df: pd.DataFrame, ticker: str, top_n: int = 15) -> pd.DataFrame:
    ...
    for i, idx in enumerate(idx_list):
        ...
        add_to_history(ticker, row.get("strike"), row.get("exp_str",""),
                       ...
                       row.get("VOI",0), ask_hit_pct, sweep_str, iv=iv_val)

        df.at[idx, "🐋 DAYS"] = get_cluster_repeat(
            ticker, row.get("strike"), row.get("exp_str",""), row.get("type","")
        )
        df.at[idx, "IV RANK"] = get_iv_rank(
            ticker, row.get("strike"), row.get("exp_str",""),
            row.get("type",""), iv_val
        )
...
# =========================
with st.expander("📖 Manuale — Options Flow Scanner PRO v4.3"):
    st.markdown("""
## 🎯 Obiettivo del Tool
    ...
    if wl:
        ...
        if st.button("🔄 Aggiorna tutti i contratti in watchlist", key="wl_refresh"):
            ...
            for i, entry in enumerate(wl):
                ...
                # Fetch dati aggiornati per questo contratto
                underlying = get_stock_price(t)
                if underlying is None:
                    wl_results.append({
                        "Ticker": t, "Contratto": f"{t} {exp} {strike}{ctype}",
                        "Nota": note, "MID": "—", "VOI": "—", "IV": "—",
                        "IV RANK": "—", "🐋 DAYS": "—", "Aggiunto": added,
                        "Underlying": "—"
                    })
                    continue

                # Recupera snapshot per il contratto specifico
                contract_sym = f"O:{t}{exp.replace('-','')}{ctype}{'%08d' % int(strike*1000)}"
                try:
                    r = requests.get(
                        ...
                        params={
                            ...
                            "strike_price": strike,
                            "expiration_date": exp,
                            "contract_type": "call" if ctype=="C" else "put",
                            "limit": 5
                        }, timeout=10
                    )
                    results = r.json().get("results", []) if r.status_code == 200 else []
                    # Trova il contratto esatto
                    ...
                    if match:
                        ...
                        iv_rnk = get_iv_rank(t, strike, exp, ctype, iv_pct)
                        ...
                        wl_results.append({
                            "Ticker": t, "Contratto": f"{t} {exp} {strike}{ctype}",
                            "Nota": note, "MID": "n/d", "VOI": "n/d", "IV": "n/d",
                            "IV RANK": "n/d", "🐋 DAYS": "—", "Aggiunto": added,
                            "Underlying": f"${underlying}"
                        })
                except Exception as ex:
                    wl_results.append({
                        "Ticker": t, "Contratto": f"{t} {exp} {strike}{ctype}",
                        "Nota": note, "MID": "err", "VOI":"err","IV":"err",
                        "IV RANK": "err", "🐋 DAYS": "—", "Aggiunto": added,
                        "Underlying": "err"
                    })
            ...
            # Mostra watchlist statica senza aggiornare
            wl_display = [{
                "Contratto": f"{e['ticker']} {e['expiration']} {e['strike']}{e['type']}",
                "Nota": e.get("note",""),
                "Aggiunto": e.get("added",""),
                "🐋 DAYS": get_cluster_repeat(e["ticker"], e["strike"], e["expiration"], e["type"])
            } for e in wl]
            st.dataframe(pd.DataFrame(wl_display), use_container_width=True, hide_index=True)

        # Rimozione singolo contratto
        st.markdown("---")
        wl_labels = [f"{e['ticker']} {e['expiration']} {e['strike']}{e['type']}" for e in wl]
        ...
        if st.button("🗑️ Rimuovi", key="wl_remove_btn") and to_remove != "— seleziona —":
            wl = [e for e in wl if f"{e['ticker']} {e['expiration']} {e['strike']}{e['type']}" != to_remove]
            save_watchlist(wl)
...
st.caption(
    ...
    "Nessun ordine viene eseguito automaticamente. — v4.3"
)
